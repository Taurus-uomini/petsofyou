...
router.post('/onsellchange', function (req, res, next) {
    var data = req.body;
    web3.eth.personal.unlockAccount(data.paddress,data.password,function(error,result)
    {
        if(error)
        {
        ...
        else
        {
            console.log(result);
            var MyContract = new web3.eth.Contract(abi, address);
            MyContract.methods.onsell(data.petaddr,data.is_onsell).send({"from":data.paddress, "gas": 3141592}).
            on('receipt', function(receipt){
                var is_onsell = receipt.events.Onsell.returnValues.is_onsell;
                if(is_onsell == data.is_onsell)
                {
                    var datajson = { "access_token": data.access_token, "petaddr": data.petaddr, "is_onsell": data.is_onsell, "pcoinnum": data.pcoinnum };
                    var option =
                        {
                            url: 'http://localhost:81/Users/onsellchange',
                            method: 'POST',
                            json: true,
                            headers:
                                {
                ...
                {
                    res.send({ "code": 1500, "message": "run error" });
                }
            });
        }
    });
